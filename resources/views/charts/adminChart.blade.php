@extends('layouts.smart-hr', ['title' => 'Monthly Attendance Charts'])
@section('content')
    <div class="page-wrapper">
        <div class="card">
            <div class="card-body">
                <h1><strong> {{$team->name}}</strong>  {{\Carbon\Carbon::today()->format('F')}} Attendance Spread
                    <a href="{{route('admin_chart_last_month', $team->id)}}" class="btn btn-primary float-right">View Last Month</a></h1>
                {{-- <button onclick="window.location='{{ route("admin_chart_last_month", $id) }}'" type="button" class="btn btn-primary float-right">View Last Month</button></h1> --}}
