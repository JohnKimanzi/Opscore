    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">Members in the <u>{{$designation->name}}</u> Designation</h3>
                </div>
                <div class="col-auto float-right ml-auto">
                    <a href="#" class="btn add-btn" data-toggle="modal" data-target="#allocate_employees_to_designation"><i class="fa fa-plus"></i> Add employee to this designation</a>
                </div>
            </div>
        </div>
        <!-- /Page Header -->
...
@endif
       ...
       <div class="card">
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped custom-data-table mb-0">
                    ...
                    <tbody>
                        @forelse ($designation->employees as $des_employee)
                        <tr>
                            <td>{{$loop->iteration}}</td>
                            <td>{{$des_employee->name}}</td>
                            <td class="text-right">
                               <button class="btn btn-primary" id="re_allocation_btn" role="button" data-toggle="modal" data-target="#designation_reallocation_modal" data-id="{{$des_employee->id}}">Re-allocate Employee</button>
                            </td>
                        </tr>
                        @empty
                        @endforelse
        ...
        <!-- Allocate Employees Modal to Designation-->
        <div id="allocate_employees_to_designation" class="modal custom-modal fade" role="dialog">
            <div class="modal-dialog modal-md modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"> Select employees to allocate to the {{$designation->name}} designation</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form action="{{route('employee_designation.allocate_employees', $designation)}}"  method="POST">
                            @csrf
                    <div class="form-group designation-duallist">
                        ...
                        <div class="row">
                            <div class="col-lg-5 col-sm-5">
                                <select name="edit_customleave_from" id="edit_customleave_select" class="form-control form-select" size="5" multiple="multiple">
                                    @forelse ($employees as $des_employee)
                                    <option value="{{$des_employee->id}}">{{$des_employee->name}}</option>
                                @empty
        ...
        <!-- /Allocate Employees Modal to Designation-->
@include('designation.re_allocate_emp')
@endsection
